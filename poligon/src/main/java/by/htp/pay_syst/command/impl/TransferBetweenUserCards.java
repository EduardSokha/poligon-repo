...
public class TransferBetweenUserCards implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		idUser = user.getId();
		cardFrom = Integer
				.parseInt(request.getParameter(RequestParameterName.REQ_PARAM_TRANSFER_BETWEEN_CARDS_CARD_FROM));
		...
		try {
			...
			response.sendRedirect("controller?command=GO_TO_TRANSFER_BETWEEN_CARDS");
